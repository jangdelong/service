...
;(function(win,$){
    ...
    sS.prototype.init = function(options){
        var me = this;
        me.opts = $.extend({}, {
            ul : me.$el.find('.img-main').find('ul'),        // 父dom
            li : me.$el.find('.img-main').find('li'),        // 子dom
            btnLi: me.$el.find('.img-thumbnail').find('li'), // 缩略图按钮（可点击块）
            index : 0,                                       // 轮播初始值
        ...
        // 绑定点击图片缩略图事件
        me.opts.btnLi.on('click', function() {
            var $this  = $(this),
                _index = $this.index();
            me._index  = _index;
